...
from eval_utils import get_metric_statistics, transform
from common.paramUtil import bfa_style_inv_enumerator, kinematic_chain
from common.quaternion import cont6d_to_mat
import numpy as np
...
bfa_data_root = "/data/chenkerui1/motion_transfer_data/processed_bfa"
cmu_data_root = "/data/chenkerui1/motion_transfer_data/processed_cmu"
...
decoder = Net.Generator(n_conv, n_up, opt.dim_z, g_channels, dim_style)
...
gm_regressor = Net.GlobalRegressor(3, 2, channels)
gm_regressor.to(opt.device)
checkpoint = torch.load(pjoin(opt.checkpoints_dir, "cmu", "GLR_CV3_NP5_NS5_FT1", "model", "best.tar"),
                            map_location=opt.device)
gm_regressor.load_state_dict(checkpoint["regressor"])
gm_regressor.eval()
...
style_cls = Net.ResNetClassifier(e_mid_channels, e_st_channels, opt.num_of_style)
style_cls.to(opt.device)
checkpoint = torch.load(
    pjoin(opt.checkpoints_dir, "bfa", "CLS_FT1_ML160", "model", "best.tar"),
    map_location=opt.device)
style_cls.load_state_dict(checkpoint["classifier"])
style_cls.eval()
...
for t in range(opt.repeat_times):
    ...
    for i, data in enumerate(data_loader):
        ...
        M2 = transform(M2.float().to(opt.device), tc_std, tc_mean, tc_mean_cls, tc_std_cls).permute(0, 2, 1)
        TM = transform(TM.float().to(opt.device), tc_std, tc_mean, tc_mean_cls, tc_std_cls).permute(0, 2, 1)
...
for key, value in res.items():
    mean, confInt = get_metric_statistics(value, opt.repeat_times)
